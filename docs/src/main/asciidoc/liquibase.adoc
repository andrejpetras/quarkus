...
https://www.liquibase.org/[Liquibase] is the leading open source tool for database schema change management.
...
[source,java]
--
import org.quarkus.liquibase.Liquibase <1>
...
<1> The Quarkus extension wraps the Liquibase object in its own implementation
<2> Inject the Quarkus liquibase object if you want to use the liquibase methods directly
...
NOTE: The Liquibase object is the instance of the org.quarkus.liquibase.Liquibase class. This kind of wrapper object to handle the
JDBC connection for the liquibase.Liquibase object. The instance open and close the database connection for each method separately.
...
[source,java]
--
import org.quarkus.liquibase.Liquibase
...
public class MigrationService {
    ...
    Liquibase liquibaseForInventory;

    @Inject
    @Named("liquibase_users") <3>
    Liquibase liquibaseForUsers;
    ...
    public void checkMigration() {
        ...
        liquibase.update();
        // Get the list of liquibase change set statuses
        List<ChangeSetStatus> status = liquibase.getChangeSetStatuses(); <5>
    }
}
--
